...
\vspace{5pt}
    ...
    \footnotesize
\noindent
In the spirit of a newly acquired DevOps culture, a large amount of companies are implementing configuration management systems (CMS). These systems allow Enterprises to provide their IT services in a predictable and consistent manner. When companies using distinct systems decide to merge, it is common that a single system is chosen to manage the full IT landscape. However, migrating between CMS is a delicate task and is heavily dependent on the tool in place. In this paper we explore the architectural models of the configuration management tools Puppet, Ansible and \texttt{mgmt}. We evaluate the applicability of each system and present a strategy for performing an in place migration of a generic node between Puppet and Ansible. Migrations between Puppet and Ansible in both directions are performed and the required steps to automate this process are documented. Migrating a node between these systems is deemed feasible, besides the chosen approach another approach for migrating is discussed at the end of this paper.
