...
\vspace{5pt}
    ...
    \footnotesize
\noindent
Migrating between configuration management systems is a delicate task which is heavily dependent on the tool in place. This paper presents a strategy for performing an automated in-place migration of a generic node between Puppet and Ansible from a corporate merge standpoint. The architectural models of established configuration management systems Puppet and Ansible are explored. Additionally prototype \texttt{mgmt} is examined. Additionally the applicability of \texttt{mgmt} in its current state is discussed. Migrations between Puppet and Ansible in both directions are performed in a test environment and the required steps to automate this process are documented. The resulting methodology allows for a clean migration during production hours with constant control over the nodes. \texttt{Mgmt} exhibits significant improvements over traditional configuration management systems but is not deemed ready for a production deployment at this point. 
     \end{abstract}
\end{minipage}
\begin{center}
  \vspace{.8cm}
